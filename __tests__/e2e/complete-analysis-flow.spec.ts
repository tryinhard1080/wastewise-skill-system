...
import { waitForJobCompletion, uploadFileViaUI, downloadFile } from './utils/test-helpers'
import * as path from 'path'

test.describe('Complete Analysis Workflow', () => {
  test.setTimeout(15 * 60 * 1000) // 15 minutes for complete workflow

  test('User can complete full analysis from login to download', async ({ authenticatedPage }) => {
    ...
    await expect(page.locator('text=/2.*files?.*uploaded/i')).toBeVisible({ timeout: 10000 })
    ...
    await page.waitForURL(/\/projects\/[a-f0-9-]+\/processing/, { timeout: 10000 })
    ...
    await expect(page.locator('text=/executive summary|overview/i')).toBeVisible()
    await expect(page.locator('text=/optimization|recommendation/i')).toBeVisible()
  ...
  test('Handles file upload errors gracefully', async ({ testProject }) => {
    ...
    await expect(page.locator('text=/invalid|error|supported/i')).toBeVisible({ timeout: 10000 })

    console.log('✅ File upload error handling test passed!')
  })

  test('Displays progress updates during analysis', async ({ seededProject }) => {
    const { page, projectId } = seededProject
    ...
    await expect(page.locator('[data-testid="progress-bar"], text=/progress|processing/i')).toBeVisible({ timeout: 30000 })
    ...
    const updatedProgress = await page.locator('[data-testid="progress-percent"]').textContent()
