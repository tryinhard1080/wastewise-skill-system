...
test.describe("Project Management", () => {
  test.describe("Project Creation", () => {
    test("User can create project with all required fields", async ({
      authenticatedPage,
    }) => {
      ...
      await expect(
        page.locator("text=Sunset Gardens Apartments"),
      ).toBeVisible();
      await expect(page.locator("text=/180.*units/i")).toBeVisible();
    });

    test("User cannot create project with missing required fields", async ({
      authenticatedPage,
    }) => {
      const page = authenticatedPage;
      ...
      await expect(
        page.locator("text=/property.*name.*required|required/i"),
      ...
      await expect(
        page.locator("text=/units.*required|equipment.*required/i"),
      ).toBeVisible({ timeout: 5000 });
  ...
  test.describe("Project Listing", () => {
    test("User can view project list on dashboard", async ({ testProject }) => {
      ...
      await expect(
        page.locator('[data-testid="project-list"], .project-list'),
      ).toBeVisible({ timeout: 5000 });
  ...
  test.describe("File Upload", () => {
    test("User can upload files to project", async ({ testProject }) => {
      ...
      await expect(
        page.locator("text=/uploaded|upload.*success|file.*added/i"),
  ...
  test.describe("Project Deletion", () => {
    test("User can delete project with confirmation", async ({
      authenticatedPage,
    }) => {
      const page = authenticatedPage;
      ...
      // Click delete button
      await page.click(
        'button:has-text("Delete"), [data-testid="delete-project"]',
      );

      // Should show confirmation dialog
      await expect(
        page.locator("text=/confirm.*delete|are.*you.*sure/i"),
      ).toBeVisible({ timeout: 5000 });
      ...
      await expect(page.locator("text=/not.*found|doesn.*exist/i")).toBeVisible(
        { timeout: 5000 },
      );
    });
  });
  ...
  test.describe("Multiple Projects", () => {
    ...
    test("Project list pagination works correctly", async ({
      authenticatedPage,
    }) => {
      const page = authenticatedPage;
      ...
      if (hasPagination) {
        ...
        await expect(page.locator("text=/Pagination Test 1[12]/i")).toBeVisible(
          { timeout: 5000 },
        );

        // Click previous page
        await page.click(
          '[data-testid="prev-page"], button:has-text("Previous")',
        );

        // Should see first page projects again
        await expect(page.locator("text=/Pagination Test [1-9]/i")).toBeVisible(
          { timeout: 5000 },
        );
      }
    });
