...
test.describe("Results & Reports", () => {
  ...
  test.describe("Results Page Display", () => {
    test("Results page displays analysis summary correctly", async ({
      seededProject,
    }) => {
      ...
      await expect(
        page.locator("text=/executive.*summary|summary|overview/i"),
      ).toBeVisible();

      // Verify key metrics are displayed
      await expect(
        page.locator("text=/total.*cost|monthly.*cost/i"),
      ).toBeVisible();
    ...
    test("Results page shows optimization recommendations", async ({
      seededProject,
    }) => {
      const { page, projectId } = seededProject;
      ...
      await expect(
        page.locator("text=/recommendations|optimizations|opportunities/i"),
      ).toBeVisible();

      // Verify recommendation details are shown
      const recommendationSection = page.locator(
        '[data-testid="recommendations"], .recommendations-section',
      );

      if ((await recommendationSection.count()) > 0) {
        // Should show at least one recommendation
        await expect(
          page.locator("text=/reduce.*frequency|monitor|schedule/i"),
        ).toBeVisible();
      }
    });
    ...
    test("Results page shows expense breakdown", async ({ seededProject }) => {
      ...
      await expect(
        page.locator("text=/expense.*breakdown|expenses|costs/i"),
      ).toBeVisible();

      // Verify expense categories are shown
      await expect(
        page.locator(
          "text=/service.*cost|environmental.*fee|fuel.*surcharge/i",
        ),
      ).toBeVisible();

      // Verify charts or visualizations are present
      const chartElement = page.locator(
        '[data-testid="expense-chart"], canvas, .chart-container',
      );
  ...
  test.describe("File Format Validation", () => {
    test("Downloaded Excel file is valid (.xlsx format)", async ({
      seededProject,
    }) => {
      const { page, projectId } = seededProject;
    ...
    test("Downloaded HTML file is valid (can be opened in browser)", async ({
      seededProject,
    }) => {
      const { page, projectId } = seededProject;
      ...
      // Verify page loaded
      await expect(newPage.locator("h1, h2, title")).toContainText(
        /WasteWise|Analysis|Dashboard/i,
      );
  ...
  test.describe("Empty State Handling", () => {
    test('Results page shows "No data" message when analysis has no results', async ({
      testProject,
    }) => {
      ...
      await expect(
        page.locator(
          "text=/no.*data|no.*results|analysis.*not.*run|run.*analysis/i",
        ),
      ...
      await expect(
        page.locator(
          'button:has-text("Run Analysis"), a:has-text("Run Analysis")',
        ),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Graceful Error Handling", () => {
    test("Results page handles missing data gracefully", async ({
      seededProject,
    }) => {
      const { page, projectId } = seededProject;
