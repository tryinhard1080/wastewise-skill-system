...
test.describe("Authentication Flows", () => {
  test.describe("User Registration (Signup)", () => {
    test("User can sign up with valid email and password", async ({ page }) => {
      ...
      // Cleanup: TODO - implement user deletion after signup
      // Note: For now, test users persist and should be cleaned up manually or via cron
      // Future: Import deleteTestUser from test-helpers and cleanup properly
    });
    ...
    test("User cannot sign up with weak password", async ({ page }) => {
      ...
      for (const password of weakPasswords) {
        ...
        if (!isDisabled) {
          ...
          await expect(
            page.locator(
              "text=/password.*weak|password.*strong|password.*requirements/i",
            ),
          ).toBeVisible({ timeout: 5000 });
        }
  ...
  test.describe("User Login", () => {
    ...
    test("User can reset forgotten password", async ({ page }) => {
      ...
      await expect(
        page.locator("text=/check.*email|reset.*link.*sent|email.*sent/i"),
      ).toBeVisible({ timeout: 10000 });
    });
  });

  test.describe("Session Management", () => {
    test("User is redirected to login when accessing protected routes", async ({
      page,
    }) => {
