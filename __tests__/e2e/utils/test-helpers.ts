...
export async function deleteTestUser(userId: string): Promise<void> {
  ...
  await supabase.from("analysis_jobs").delete().eq("user_id", userId);
...
export async function getCurrentJobProgress(page: Page): Promise<number> {
  const progressText = await page.textContent(
...
export async function waitForProgress(
  ...
  throw new Error(
    `Progress did not reach ${targetPercent}% within ${timeout}ms`,
  );
}
