...
test.describe('Error Handling & Edge Cases', () => {
  test.describe('API Errors', () => {
    test('User sees error message when API is unavailable', async ({ page }) => {
      ...
      try {
        ...
        await expect(page.locator('text=/service.*unavailable|unable.*to.*load|try.*again.*later/i')).toBeVisible({ timeout: 10000 })
      } finally {
    ...
    test('User sees error message when file upload fails', async ({ page }) => {
      ...
      try {
        ...
        await expect(page.locator('text=/upload.*failed|error.*uploading/i')).toBeVisible({ timeout: 10000 })
      } finally {
        await deleteTestUser(userId)
      }
    })
    ...
    test('User sees error message when job fails', async ({ page }) => {
      ...
      try {
        ...
        if (await analyzeButton.count() > 0 && !(await analyzeButton.isDisabled())) {
          await analyzeButton.click()

          // Should show error message
          await expect(page.locator('text=/analysis.*failed|error.*analyzing|failed.*to.*analyze/i')).toBeVisible({ timeout: 10000 })
        }
      } finally {
        await deleteTestUser(userId)
      }
      ...
      try {
        ...
        if (await errorMessage.count() > 0) {
          ...
          if (await retryButton.count() > 0) {
            ...
            await expect(page.locator('text=/error|failed/i')).not.toBeVisible({ timeout: 5000 })
          }
        }
      } finally {
        await deleteTestUser(userId)
  ...
  test.describe('User-Friendly Messages', () => {
    test('User sees meaningful error messages (not technical jargon)', async ({ page }) => {
      const testEmail = `test-${Date.now()}@wastewise.test`
      const testPassword = 'TestPassword123!'
      const userId = await createTestUser(testEmail, testPassword)
  ...
  test.describe('Network Errors', () => {
    test('App handles network errors gracefully', async ({ page }) => {
      ...
      try {
        ...
        await expect(page.locator('text=/network.*error|connection.*error|timeout|unable.*to.*connect/i')).toBeVisible({ timeout: 15000 })
  ...
  test.describe('Rate Limiting', () => {
    test('App handles rate limiting gracefully (429 response)', async ({ page }) => {
      const testEmail = `test-${Date.now()}@wastewise.test`
      const testPassword = 'TestPassword123!'
      const userId = await createTestUser(testEmail, testPassword)
      ...
      try {
        ...
        if (await analyzeButton.count() > 0 && !(await analyzeButton.isDisabled())) {
          ...
          await expect(page.locator('text=/too.*many.*requests|rate.*limit|try.*again.*later|wait.*before/i')).toBeVisible({ timeout: 10000 })
        }
      } finally {
        await deleteTestUser(userId)
      }
  ...
  test.describe('Validation Errors', () => {
    test('App handles validation errors with field-specific messages', async ({ page }) => {
      const testEmail = `test-${Date.now()}@wastewise.test`
      const testPassword = 'TestPassword123!'
      const userId = await createTestUser(testEmail, testPassword)
      ...
      try {
        ...
        const propertyNameError = page.locator('text=/property.*name.*required/i')
        const unitsError = page.locator('text=/units.*must.*be.*positive|invalid.*units/i')
