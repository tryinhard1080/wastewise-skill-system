...
test.describe('File Upload & Validation', () => {
  test.describe('Supported File Types', () => {
    test('User can upload PDF invoice', async ({ testProject }) => {
      ...
      try {
        ...
        await expect(page.locator('text=/uploaded|upload.*success/i')).toBeVisible({ timeout: 10000 })
    ...
    test('User can upload Excel invoice (.xlsx)', async ({ testProject }) => {
      ...
      const excelPath = path.join(__dirname, 'seeds/test-files/sample-invoice.xlsx')
  ...
  test.describe('Multiple File Upload', () => {
    test('User can upload multiple files at once', async ({ testProject }) => {
      ...
      const haulLogPath = path.join(__dirname, 'seeds/test-files/sample-haullog.xlsx')
      ...
      // Wait for upload success
      await expect(page.locator('text=/2.*files?.*uploaded|uploaded.*2.*files?/i')).toBeVisible({ timeout: 15000 })
  ...
  test.describe('File Size Validation', () => {
    test('User cannot upload files exceeding size limit (10MB)', async ({ testProject }) => {
      const { page, projectId } = testProject
      ...
      try {
        ...
        await expect(page.locator('text=/file.*too.*large|exceeds.*size.*limit|maximum.*10.*mb/i')).toBeVisible({ timeout: 10000 })
  ...
  test.describe('File Type Validation', () => {
    test('User cannot upload unsupported file types', async ({ testProject }) => {
      const { page, projectId } = testProject
      ...
      for (const file of unsupportedFiles) {
        ...
        try {
          ...
          await expect(page.locator('text=/unsupported.*file.*type|invalid.*file.*type|file.*type.*not.*allowed/i')).toBeVisible({ timeout: 10000 })
  ...
  test.describe('Upload Progress', () => {
    test('User sees upload progress indicator', async ({ testProject }) => {
      ...
      try {
        ...
        const progressIndicator = page.locator('[data-testid="upload-progress"], .upload-progress, text=/uploading/i')
        await expect(progressIndicator).toBeVisible({ timeout: 5000 })

        // Wait for completion
        await expect(page.locator('text=/uploaded|upload.*complete/i')).toBeVisible({ timeout: 15000 })
      } finally {
        // Cleanup
  ...
  test.describe('File Management', () => {
    test('User can remove uploaded files before analysis', async ({ testProject }) => {
      const { page, projectId } = testProject
      ...
      // Wait for upload
      await expect(page.locator('text=/sample-invoice/i')).toBeVisible({ timeout: 10000 })

      // Click remove button for this file
      const removeButton = page.locator('[data-testid="remove-file"], button:has-text("Remove")').first()
      await removeButton.click()

      // File should disappear from list
      await expect(page.locator('text=/sample-invoice/i')).not.toBeVisible({ timeout: 5000 })
    })

    test('User sees file list with names and sizes', async ({ testProject }) => {
      const { page, projectId } = testProject
      ...
      const fileTypeIndicator = page.locator('[data-testid="file-type"], .file-type, text=/.xlsx/i')
