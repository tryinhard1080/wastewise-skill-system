...
test.describe('Analysis Workflows', () => {
  ...
  test.describe('Complete Analysis Job', () => {
    test('Complete analysis job completes successfully', async ({ seededProject }) => {
      ...
      // Start analysis
      await page.click('button:has-text("Analyze"), button:has-text("Start Analysis")')

      // Should redirect to processing page
      await page.waitForURL(/\/processing/, { timeout: 10000 })

      // Progress indicators should be visible
      await expect(page.locator('[data-testid="progress-bar"], text=/processing|analyzing/i')).toBeVisible({ timeout: 30000 })
      ...
      while (Date.now() - startTime < maxWaitTime && !completed) {
        ...
        const completedIndicator = page.locator('text=/completed|finished|done/i')
  ...
  test.describe('Invoice Extraction Job', () => {
    test('Invoice extraction job completes successfully', async ({ testProject }) => {
      ...
      await expect(page.locator('text=/uploaded/i')).toBeVisible({ timeout: 10000 })

      // Start invoice extraction (if there's a separate button for this)
      // Otherwise it will be part of the complete analysis
      const extractButton = page.locator('button:has-text("Extract"), button:has-text("Extract Invoices")')
      ...
      if (await extractButton.count() > 0) {
        ...
        // Wait for completion
        await expect(page.locator('text=/extraction.*complete|completed/i')).toBeVisible({ timeout: 2 * 60 * 1000 })
      } else {
  ...
  test.describe('Regulatory Research Job', () => {
    test('Regulatory research job completes successfully', async ({ testProject }) => {
      const { page, projectId } = testProject

      await page.goto(`/projects/${projectId}`)

      // Check if there's a separate regulatory research button
      const researchButton = page.locator('button:has-text("Research"), button:has-text("Regulatory Research")')
      ...
      if (await researchButton.count() > 0) {
        ...
        while (Date.now() - startTime < maxWaitTime && !completed) {
          const completedIndicator = page.locator('text=/research.*complete|ordinance.*found/i')
          if (await completedIndicator.count() > 0) {
            completed = true
            break
          }
  ...
  test.describe('Report Generation Job', () => {
    test('Report generation job completes successfully', async ({ seededProject }) => {
      const { page, projectId } = seededProject
      ...
      const resultsLink = page.locator('a:has-text("View Results"), button:has-text("View Results")')
      ...
      await expect(page.locator('button:has-text("Download Excel"), a:has-text("Download Excel")')).toBeVisible({ timeout: 30000 })
      await expect(page.locator('button:has-text("Download HTML"), a:has-text("Download HTML")')).toBeVisible({ timeout: 30000 })
  ...
  test.describe('Progress Tracking', () => {
    test('Job shows progress updates during processing', async ({ seededProject }) => {
      const { page, projectId } = seededProject
      ...
      await expect(page.locator('[data-testid="progress-bar"], .progress-bar')).toBeVisible({ timeout: 30000 })
    ...
    test('User can view job status on processing page', async ({ seededProject }) => {
      const { page, projectId } = seededProject
      ...
      await expect(page.locator('[data-testid="job-status"], text=/status/i')).toBeVisible({ timeout: 10000 })

      // Verify current step is shown
      await expect(page.locator('[data-testid="current-step"], text=/extracting|analyzing|generating/i')).toBeVisible({ timeout: 30000 })

      // Verify progress percentage is shown
      await expect(page.locator('[data-testid="progress-percent"]')).toBeVisible({ timeout: 10000 })
    })
  })

  test.describe('Job Completion', () => {
    test('User is redirected to results page when job completes', async ({ seededProject }) => {
      const { page, projectId } = seededProject
      ...
      await expect(page.locator('h1, h2')).toContainText(/results|analysis|complete/i)
    })
  })
  ...
  test.describe('Error Handling', () => {
    test('Failed job shows error message', async ({ testProject }) => {
      ...
      if (await analyzeButton.count() > 0) {
        ...
        await expect(page.locator('text=/no.*files|files.*required|upload.*files/i')).toBeVisible({ timeout: 10000 })
      } else {
        // Button might be disabled when no files
        const disabledButton = page.locator('button:has-text("Analyze"):disabled')
  ...
  test.describe('Job Cancellation', () => {
    test('User can cancel running job', async ({ seededProject }) => {
      ...
      const cancelButton = page.locator('button:has-text("Cancel"), [data-testid="cancel-job"]')
      ...
      if (await cancelButton.count() > 0) {
        ...
        await expect(page.locator('text=/cancelled|canceled|stopped/i')).toBeVisible({ timeout: 10000 })
      } else {
