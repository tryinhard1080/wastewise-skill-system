...
test.describe('Performance & Load Testing', () => {
  test.describe('Page Load Performance', () => {
    ...
    test('Project list loads in <2 seconds', async ({ authenticatedPage }) => {
      ...
      await expect(page.locator('[data-testid="project-list"], text=/no.*projects|create.*project/i')).toBeVisible()
  ...
  test.describe('File Upload Performance', () => {
    test('File upload completes in <30 seconds (10MB file)', async ({ testProject }) => {
      ...
      try {
        ...
        await expect(page.locator('text=/uploaded|upload.*success/i')).toBeVisible({ timeout: 30000 })
  ...
  test.describe('Analysis Performance', () => {
    test('Analysis job completes in <5 minutes (realistic dataset)', async ({ seededProject }) => {
      const { page, projectId } = seededProject
  ...
  test.describe('Concurrent Operations', () => {
    test('App handles 10 concurrent file uploads', async ({ testProject }) => {
      ...
      try {
        ...
        await expect(page.locator('text=/10.*files?.*uploaded|uploaded.*10/i')).toBeVisible({ timeout: 60000 })
  ...
  test.describe('Large Dataset Handling', () => {
    test('App handles large datasets (250-unit property, 6 months data)', async ({ seededProject }) => {
      const { page, projectId } = seededProject
  ...
  test.describe('Loading States', () => {
    test('App shows loading indicators during long operations', async ({ seededProject }) => {
      const { page, projectId } = seededProject
      ...
      const loadingIndicators = page.locator('[data-testid="loading"], [data-testid="progress-bar"], .spinner, text=/loading|processing/i')
