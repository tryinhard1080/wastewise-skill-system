...
describe('Sandbox Configuration', () => {
  ...
  describe('Profile Configurations', () => {
    it('should have all 4 profile files', async () => {
      ...
      for (const profile of profiles) {
        const profilePath = path.join(process.cwd(), '.claude', 'profiles', profile)
    ...
    it('should have valid JSON in all profiles', async () => {
      ...
      for (const profile of profiles) {
        ...
        expect(() => JSON.parse(content), `${profile} should be valid JSON`).not.toThrow()
      }
    })
  ...
  describe('Allowed Resources', () => {
    it('should allow project directories', async () => {
      ...
      const requiredDirs = ['./app/**', './components/**', './lib/**', './__tests__/**']
...
describe('Documentation', () => {
  ...
  it('should have quick reference', async () => {
    const quickRefPath = path.join(process.cwd(), '.claude', 'SANDBOX_QUICK_REF.md')
    const exists = await fs.access(quickRefPath).then(() => true).catch(() => false)
    expect(exists).toBe(true)
  })

  it('should have profile README', async () => {
    const readmePath = path.join(process.cwd(), '.claude', 'profiles', 'README.md')
