...
describe('Security Hardening - File Upload Validation', () => {
  describe('File size validation', () => {
    it('should reject files exceeding MAX_FILE_SIZE', () => {
      const oversizedFile = new File(['x'.repeat(MAX_FILE_SIZE + 1)], 'huge.pdf', {
  ...
  describe('MIME type validation', () => {
    ...
    it('should reject HTML files (XSS risk)', () => {
      const htmlFile = new File(['<script>alert("XSS")</script>'], 'malicious.html', {
        type: 'text/html',
      })
    ...
    it('should reject SVG files (XSS risk)', () => {
      const svgFile = new File(['<svg onload="alert(\'XSS\')"></svg>'], 'malicious.svg', {
        type: 'image/svg+xml',
      })
