...
const mockSearchManagerSearch = vi.fn();
...
// Mock search manager to bypass cache
vi.mock("@/lib/search/search-manager", () => ({
  getSearchManager: () => ({
    search: mockSearchManagerSearch,
  }),
}));
...
describe("RegulatoryResearchSkill", () => {
  ...
  beforeEach(() => {
    ...
    vi.clearAllMocks();
    mockSearchOrdinances.mockReset();
    mockGetContents.mockReset();
    mockAnthropicCreate.mockReset();
    mockSupabaseInsert.mockReset();
    mockSearchManagerSearch.mockReset();
  ...
  describe("ordinance search", () => {
    it("should search for ordinances and return results", async () => {
      const mockSearchManagerResponse = {
        results: [
          {
            url: "https://library.municode.com/tx/austin/codes/code_of_ordinances",
            title: "Austin Municipal Code - Chapter 15 Solid Waste",
            snippet:
              "Regulations for waste collection and disposal in multifamily properties. Recycling required for multifamily 50+ units. Minimum 2x per week collection.",
          },
        ],
      ...
      mockSearchManagerSearch.mockResolvedValue(mockSearchManagerResponse);

      // Access private method via any casting for testing
      const searchResults = await (skill as any).searchOrdinances(
        "Austin",
        "TX",
      );
      ...
      expect(searchResults[0].relevantExcerpts).toHaveLength(1);
    });

    it("should return empty array when no ordinances found", async () => {
      mockSearchManagerSearch.mockResolvedValue({
        results: [],
        provider: "exa" as const,
        cached: false,
        executionTime: 50,
    ...
    it("should handle search errors gracefully", async () => {
      mockSearchManagerSearch.mockRejectedValue(
    ...
    it("should extract requirements from ordinances using Claude", async () => {
      ...
      const mockClaudeResponse = {
        wasteRequirements: [
          {
            requirement:
              "Minimum 2x per week collection for properties 100+ units",
            mandatory: true,
  ...
  describe("compliance assessment", () => {
    ...
    it("should flag recycling requirements as MEDIUM severity", async () => {
      ...
      expect(compliance.issues[0].severity).toBe("MEDIUM");
      expect(
        compliance.recommendations.some((r: string) =>
          r.includes("cardboard, plastic"),
        ),
      ).toBe(true);
    });
        ...
        results: [
          {
            ...
            snippet: "Waste regulations for multifamily properties",
          },
        ],
        provider: "exa" as const,
        cached: false,
