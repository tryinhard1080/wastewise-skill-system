...
describe('SearchManager', () => {
  ...
  describe('Provider Fallback', () => {
    ...
    it('should fallback to second provider when first fails', async () => {
      ...
      manager.providers[0].search = vi.fn().mockRejectedValue(new Error('Provider 1 failed'))
    ...
    it('should fallback through all providers until one succeeds', async () => {
      ...
      manager.providers[0].search = vi.fn().mockRejectedValue(new Error('Failed'))
      // @ts-expect-error - Mock second to fail
      manager.providers[1].search = vi.fn().mockRejectedValue(new Error('Failed'))
