...
describe('Logger', () => {
  ...
  describe('Context and Data', () => {
    ...
    it('should include structured data', () => {
      logger.info('API request', undefined, { method: 'POST', path: '/api/analyze' })
    ...
    it('should include both context and data', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"duration_ms":234'))
    })
  })
  ...
  describe('Child Logger', () => {
    it('should create child logger with fixed context', () => {
      const childLogger = logger.child({ jobId: 'job-123', skillName: 'test-skill' })

      childLogger.info('Child log message')

      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"jobId":"job-123"'))
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"skillName":"test-skill"'))
    })
    ...
    it('should merge additional context in child logger', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"jobId":"job-123"'))
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"userId":"456"'))
    })
  })
    ...
    it('should send warnings as breadcrumbs in production', () => {
      ...
      expect(Sentry.addBreadcrumb).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          message: 'Warning occurred',
          level: 'warning',
          data: expect.objectContaining({ userId: '123', reason: 'rate limit' }),
        })
      )
    })
