...
describe('Database Synchronization', () => {
  it('constants should match skills_config seed values', () => {
    ...
    expect(COMPACTOR_OPTIMIZATION_THRESHOLD).toBe(expectedThresholds.compactor_tons);
    expect(CONTAMINATION_THRESHOLD_PCT).toBe(expectedThresholds.contamination_pct / 100);
    expect(BULK_SUBSCRIPTION_THRESHOLD).toBe(expectedThresholds.bulk_monthly);
    expect(LEASEUP_VARIANCE_THRESHOLD).toBe(expectedThresholds.leaseup_variance);
  });
});
