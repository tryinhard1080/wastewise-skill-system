...
describe.skipIf(!runIntegrationTests)(
  "RegulatoryResearchSkill - Integration Tests",
  () => {
    ...
    beforeAll(() => {
      if (!runIntegrationTests) {
        ...
        console.log(
          "   Set EXA_API_KEY and ANTHROPIC_API_KEY to run these tests",
        );
      }
    });
    ...
    describe("Austin, TX ordinances", () => {
      it(
        "should find and extract Austin waste management ordinances",
        { timeout: 60000 },
        async () => {
          ...
          console.log(
            `   Ordinances found: ${result.data?.ordinances.length || 0}`,
          );
          console.log(
            `   Waste requirements: ${result.data?.requirements.waste.length || 0}`,
          );
          console.log(
            `   Recycling requirements: ${result.data?.requirements.recycling.length || 0}`,
          );
          console.log(
            `   Compliance status: ${result.data?.compliance.status}`,
          );
          ...
          if (result.data?.ordinances && result.data.ordinances.length > 0) {
            ...
            result.data.ordinances.forEach((ord, idx) => {
              ...
              console.log(
                `      Excerpts: ${ord.relevantExcerpts.slice(0, 2).join("; ")}`,
              );
            });
          }
    ...
    describe("Chicago, IL ordinances", () => {
      it(
        "should find and extract Chicago waste management ordinances",
        { timeout: 60000 },
        async () => {
    ...
    describe("edge cases", () => {
      it(
        "should handle small town with no ordinances gracefully",
        { timeout: 60000 },
        async () => {
    ...
    describe("performance", () => {
      it(
        "should complete within reasonable time (<90 seconds)",
        { timeout: 120000 },
        async () => {
