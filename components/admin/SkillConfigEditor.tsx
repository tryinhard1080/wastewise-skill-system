...
export function SkillConfigEditor({ config, onSave }: SkillConfigEditorProps) {
  ...
  const handleChange = (key: string, value: number, type: 'conversion_rates' | 'thresholds') => {
    setFormData((prev) => ({
      ...prev,
      [type]: {
        ...prev[type],
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <Alert>
          ...
          <AlertDescription>
            Changing formula constants requires running the full evaluation suite to ensure
            calculations remain accurate. All changes must be validated before deployment.
          </AlertDescription>
        </Alert>

        {/* Formula Constants */}
        <div className="space-y-4">
          <h3 className="font-semibold text-sm text-gray-700">Formula Constants</h3>
          {constants.map((constant) => {
            const type = constant.key.includes('ypd') || constant.key.includes('capacity')
              ? 'conversion_rates'
              : 'thresholds'
