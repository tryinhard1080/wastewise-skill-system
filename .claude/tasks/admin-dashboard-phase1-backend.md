...
async function handleGET(req: NextRequest & { adminUserId: string }) {
  try {
    ...
    const stats = {
      ...
      processing:
        statusCounts?.filter((j) => j.status === "processing").length || 0,
      completed:
        statusCounts?.filter((j) => j.status === "completed").length || 0,
      failed: statusCounts?.filter((j) => j.status === "failed").length || 0,
      cancelled:
        statusCounts?.filter((j) => j.status === "cancelled").length || 0,
    };
