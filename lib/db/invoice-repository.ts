...
export class InvoiceRepository {
  ...
  async batchInsert(invoices: InvoiceRecord[]): Promise<BatchInsertResult> {
    ...
    for (let i = 0; i < invoices.length; i += this.BATCH_SIZE) {
      const chunk = invoices.slice(
        i,
        Math.min(i + this.BATCH_SIZE, invoices.length),
      );
  ...
  async getByProjectId(
    ...
    try {
      ...
      if (error) {
        logger.error("Failed to fetch invoices", new Error(error.message), {
  ...
  async getByProjectIdAndDateRange(
    ...
    try {
      ...
      if (error) {
        logger.error(
          "Failed to fetch invoices by date range",
  ...
  async update(
    ...
    try {
      ...
      if (updates.invoice_number !== undefined)
        updateData.invoice_number = updates.invoice_number;
      if (updates.invoice_date !== undefined)
        updateData.invoice_date = updates.invoice_date;
      if (updates.vendor_name !== undefined)
        updateData.vendor_name = updates.vendor_name;
      if (updates.service_type !== undefined)
        updateData.service_type = updates.service_type;
      ...
      if (updates.charges !== undefined)
        updateData.charges = updates.charges as any;
      ...
      if (error) {
        logger.error("Failed to update invoice", new Error(error.message), {
          id,
        });
        return { data: null, error: error.message };
      }
  ...
  async delete(
    ...
    try {
      ...
      if (error) {
        logger.error("Failed to delete invoice", new Error(error.message), {
          id,
        });
  ...
  async deleteByProjectId(
    ...
    try {
      ...
      if (error) {
        logger.error(
          "Failed to delete project invoices",
          new Error(error.message),
          { project_id: projectId },
        );
        return { success: false, error: error.message };
  ...
  async getCountByProjectId(
    ...
    try {
      ...
      if (error) {
        logger.error("Failed to count invoices", new Error(error.message), {
          project_id: projectId,
        });
