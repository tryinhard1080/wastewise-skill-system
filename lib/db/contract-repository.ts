...
export class ContractRepository {
   ...
   */
  async upsert(contract: ContractRecord): Promise<{ data: ContractRow | null; error: string | null }> {
    try {
   ...
   */
  async create(contract: ContractRecord): Promise<{ data: ContractRow | null; error: string | null }> {
    try {
  ...
  async update(
    ...
    try {
      ...
      if (updates.source_file_id !== undefined) updateData.source_file_id = updates.source_file_id
      ...
      if (updates.term_length_years !== undefined) updateData.term_length_years = updates.term_length_years
      if (updates.clauses !== undefined) updateData.clauses = updates.clauses as any
      ...
      if (error) {
        logger.error('Failed to update contract', new Error(error.message), { id })
        return { data: null, error: error.message }
      }
   ...
   */
  async getByProjectId(projectId: string): Promise<{ data: ContractRow | null; error: string | null }> {
    try {
      const { data, error } = await this.supabase
        .from('contract_terms')
        .select('*')
      ...
      if (error) {
        ...
        logger.error('Failed to fetch contract', new Error(error.message), { project_id: projectId })
        return { data: null, error: error.message }
      }
  ...
  async getExpiringContracts(
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to fetch expiring contracts', new Error(error.message), {
  ...
  async delete(id: string): Promise<{ success: boolean; error: string | null }> {
    try {
      const { error } = await this.supabase.from('contract_terms').delete().eq('id', id)

      if (error) {
        logger.error('Failed to delete contract', new Error(error.message), { id })
  ...
  async deleteByProjectId(projectId: string): Promise<{ success: boolean; error: string | null }> {
    try {
      const { error } = await this.supabase.from('contract_terms').delete().eq('project_id', projectId)

      if (error) {
        logger.error('Failed to delete project contract', new Error(error.message), { project_id: projectId })
        return { success: false, error: error.message }
      }
