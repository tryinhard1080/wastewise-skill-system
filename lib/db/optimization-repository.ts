...
type OptimizationInsert = Database['public']['Tables']['optimizations']['Insert']
type OptimizationUpdate = Database['public']['Tables']['optimizations']['Update']
...
export class OptimizationRepository {
  ...
  async create(
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to create optimization', new Error(error.message), {
  ...
  async batchCreate(
    ...
    try {
      ...
      const { data, error } = await this.supabase.from('optimizations').insert(inserts).select()

      if (error) {
        logger.error('Failed to batch create optimizations', new Error(error.message), {
   ...
   */
  async getByProjectId(projectId: string): Promise<{ data: OptimizationRow[]; error: string | null }> {
    try {
      const { data, error } = await this.supabase
        .from('optimizations')
        .select('*')
      ...
      if (error) {
        logger.error('Failed to fetch optimizations', new Error(error.message), { project_id: projectId })
        return { data: [], error: error.message }
      }
  ...
  async getRecommendedByProjectId(
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to fetch recommended optimizations', new Error(error.message), {
  ...
  async getByType(
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to fetch optimizations by type', new Error(error.message), {
          project_id: projectId,
          type,
        })
        return { data: [], error: error.message }
  ...
  async update(
    ...
    try {
      ...
      if (updates.contact_info !== undefined) updateData.contact_info = updates.contact_info as any
      ...
      if (error) {
        logger.error('Failed to update optimization', new Error(error.message), { id })
        return { data: null, error: error.message }
      }
  ...
  async delete(id: string): Promise<{ success: boolean; error: string | null }> {
    try {
      const { error } = await this.supabase.from('optimizations').delete().eq('id', id)

      if (error) {
        logger.error('Failed to delete optimization', new Error(error.message), { id })
  ...
  async deleteByProjectId(projectId: string): Promise<{ success: boolean; error: string | null }> {
    try {
      const { error } = await this.supabase.from('optimizations').delete().eq('project_id', projectId)

      if (error) {
        logger.error('Failed to delete project optimizations', new Error(error.message), { project_id: projectId })
        return { success: false, error: error.message }
      }
   ...
   */
  async getTotalSavings(projectId: string): Promise<{ total: number; error: string | null }> {
    try {
      const { data, error } = await this.supabase
        .from('optimizations')
      ...
      if (error) {
        logger.error('Failed to calculate total savings', new Error(error.message), { project_id: projectId })
      ...
      const total = data.reduce((sum: number, opt: { calculation_breakdown: unknown }) => {
