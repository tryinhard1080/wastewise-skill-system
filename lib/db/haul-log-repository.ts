...
export class HaulLogRepository {
  ...
  async create(
    haul: HaulLogRecord,
  ): Promise<{ data: HaulLogRow | null; error: string | null }> {
    try {
      ...
      if (error) {
        logger.error(
          "Failed to create haul log entry",
  ...
  async batchInsert(hauls: HaulLogRecord[]): Promise<BatchInsertResult> {
    ...
    for (let i = 0; i < enrichedHauls.length; i += this.BATCH_SIZE) {
      const chunk = enrichedHauls.slice(
        i,
        Math.min(i + this.BATCH_SIZE, enrichedHauls.length),
      );
  ...
  private calculateDaysSinceLast(hauls: HaulLogRecord[]): HaulLogRecord[] {
    ...
    const sorted = [...hauls].sort(
      (a, b) =>
        new Date(a.haul_date).getTime() - new Date(b.haul_date).getTime(),
    );
  ...
  async getByProjectId(
    ...
    try {
      ...
      if (error) {
        logger.error("Failed to fetch haul logs", new Error(error.message), {
  ...
  async getByProjectIdAndDateRange(
    ...
    try {
      ...
      if (error) {
        logger.error(
          "Failed to fetch haul logs by date range",
          new Error(error.message),
          {
            project_id: projectId,
  ...
  async getCountByProjectId(
    ...
    try {
      ...
      if (error) {
        logger.error("Failed to count haul logs", new Error(error.message), {
          project_id: projectId,
        });
   ...
   */
  async getAverageTonnage(
    projectId: string,
  ): Promise<{ average: number; error: string | null }> {
    try {
      const { data, error } = await this.supabase
        .from("haul_log")
        .select("tonnage")
      ...
      if (error) {
        logger.error(
          "Failed to calculate average tonnage",
          new Error(error.message),
          { project_id: projectId },
        );
      ...
      const total = data.reduce(
        (sum: number, haul: { tonnage: number | null }) =>
          sum + (haul.tonnage || 0),
        0,
      );
  ...
  async deleteByProjectId(
    ...
    try {
      ...
      if (error) {
        logger.error(
          "Failed to delete project haul logs",
          new Error(error.message),
          { project_id: projectId },
        );
