...
export class CompactorOptimizationSkill extends BaseSkill<CompactorOptimizationResult> {
  ...
  async validate(context: SkillContext): Promise<ValidationResult> {
    const validationLogger = logger.child({ skillName: this.name, projectId: context.projectId })
    ...
    if (!context.haulLog || context.haulLog.length === 0) {
      errors.push({
        field: 'haulLog',
        message: 'No haul log data found. Compactor optimization requires haul records.',
  ...
  protected async executeInternal(
    ...
    const totalTons = haulLog.reduce((sum, haul) => {
      const tonnage = typeof haul.tonnage === 'string'
    ...
    executionLogger.debug('Pickup intervals calculated', {
      maxDaysBetween,
      avgDaysBetween: daysBetween.length > 0
    ...
    if (recommend) {
      ...
      const currentAnnualHauls = Math.round((totalHauls / haulLog.length) * 365)
      ...
      const avgCostPerHaul = context.invoices.length > 0
        ? totalInvoiceAmount / context.invoices.reduce((sum, inv) => sum + (inv.hauls || 0), 0)
      ...
      const roiPercent = totalInvestment > 0 ? (netYear1Savings / totalInvestment) * 100 : 0

      // Calculate payback period
      const paybackMonths = totalInvestment > 0
      ...
      executionLogger.info('No monitoring recommendation', {
        reason: avgTonsPerHaul >= COMPACTOR_OPTIMIZATION_THRESHOLD
