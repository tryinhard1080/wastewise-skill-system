...
export class WasteWiseAnalyticsSkill extends BaseSkill<WasteWiseAnalyticsCompleteResult> {
  ...
  async validate(context: SkillContext): Promise<ValidationResult> {
    ...
    if (!context.invoices || context.invoices.length === 0) {
      errors.push({
        field: "invoices",
        message:
          "No invoice data found. At least one invoice is required for analysis.",
  ...
  protected async executeInternal(
    ...
    if (leaseUpDetected) {
      executionLogger.warn(
        "Lease-up detected - limiting optimization recommendations",
      );
    }
    ...
    if (this.shouldRunCompactorOptimization(context)) {
      ...
      try {
        ...
        if (!compactorSkill) {
          executionLogger.warn(
            "Compactor optimization skill not found in registry",
          );
        } else {
          ...
          if (result.success && result.data) {
            ...
            if (result.metadata.aiUsage) {
              ...
              aiUsageTracker.totalTokensInput +=
                result.metadata.aiUsage.tokensInput;
              aiUsageTracker.totalTokensOutput +=
                result.metadata.aiUsage.tokensOutput;
    ...
    if (context.project?.city && context.project?.state) {
      try {
        ...
        if (!regulatorySkill) {
          executionLogger.warn(
            "Regulatory research skill not found in registry",
          );
        } else {
          ...
          if (result.success && result.data) {
            ...
            if (regulatoryCompliance.compliance?.status === "NON_COMPLIANT") {
              const complianceIssues =
                regulatoryCompliance.compliance.issues || [];
              ...
              complianceIssues.forEach((issue: any, index: number) => {
                if (issue.severity === "HIGH" || issue.severity === "MEDIUM") {
                  recommendations.push({
                    ...
                    confidence:
                      regulatoryCompliance.confidence === "HIGH"
                        ? "HIGH"
                        : "MEDIUM",
                  });
                }
    ...
    const reports = await this.generateReports(context, {
      invoiceMetrics,
    ...
    const optimizedMonthlyCost =
      currentMonthlyCost - totalSavingsPotential / 12; // Annual to monthly
    const savingsPercentage =
      currentMonthlyCost > 0
        ? (totalSavingsPotential / (currentMonthlyCost * 12)) * 100
        : 0;
  ...
  private calculateInvoiceMetrics(context: SkillContext): InvoiceMetrics {
    ...
    const costPerDoor = calculateCostPerDoor(
      totalSpend / invoices.length,
      context.project.units,
    );
    ...
    if (context.project.equipment_type === "COMPACTOR") {
      const totalTons = invoices.reduce((sum, inv) => {
        const tons =
          typeof inv.tonnage === "string"
            ? parseFloat(inv.tonnage)
            : inv.tonnage || 0;
        return sum + tons;
      }, 0);
      yardsPerDoor = calculateCompactorYardsPerDoor(
        totalTons,
        context.project.units,
      );
    } else {
    ...
    const contaminationPercent =
      totalSpend > 0 ? (contaminationSpend / totalSpend) * 100 : 0;
  ...
  private detectLeaseUp(
    ...
    const variance =
      ((metrics.yardsPerDoor - benchmarkYardsPerDoor) / benchmarkYardsPerDoor) *
      100;
  ...
  private checkContaminationReduction(
    ...
    if (contaminationPct > CONTAMINATION_THRESHOLD_PCT) {
      ...
      return {
        ...
        implementation:
          "Partner with Ally Waste or similar service for resident education",
        confidence: "MEDIUM",
      };
    }
  ...
  private async generateReports(
    ...
    try {
      ...
      const { generateExcelReport, generateHtmlDashboard, uploadReports } =
        ...
        summary: {
          ...
          savingsPercentage:
            analysisData.invoiceMetrics.totalSpend > 0
              ? (totalSavingsPotential /
                  (analysisData.invoiceMetrics.totalSpend * 12)) *
                100
              : 0,
      ...
      executionLogger.warn(
        "Returning placeholder report data due to generation failure",
      );

      const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
      const projectName = context.project.property_name.replace(
        /[^a-zA-Z0-9]/g,
        "_",
      );
