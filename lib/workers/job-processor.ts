...
export class JobProcessor {
  ...
  async processJob(jobId: string): Promise<void> {
    ...
    try {
      ...
      if (job.status !== "pending" && job.status !== "processing") {
        jobLogger.warn("Job is not in processable state, skipping", {
          status: job.status,
        });
        return;
      }
      ...
      if (job.status === "pending") {
        ...
        try {
          const { error: startError } = await this.supabase.rpc(
            "start_analysis_job",
            {
              job_id: jobId,
            },
          ...
          if (startError) {
            if (
              startError.message &&
              (startError.message.includes("function") ||
                startError.code === "42883")
            ) {
          ...
          if ((err as Error).message === "RPC_MISSING") {
            jobLogger.warn(
              "RPC function not found, falling back to manual update",
            );
            ...
            if (updateError)
              throw new Error(
                `Failed to start job (fallback): ${updateError.message}`,
              );
          } else {
            throw err;
          }
  ...
  private async handleJobFailure(jobId: string, error: Error): Promise<void> {
    ...
    try {
      ...
      if (fetchError || !job) {
        failureLogger.error(
          "Failed to fetch job for failure handling",
          fetchError as Error,
        );
        return;
      }
  ...
  private async processCompleteAnalysis(job: AnalysisJob): Promise<void> {
    ...
    const result = await executeSkillWithProgress(
      ...
      async (percent, step) => {
        ...
        try {
          const { error: progressError } = await this.supabase.rpc(
            "update_job_progress",
            {
              job_id: job.id,
          ...
          if (progressError) {
            if (
              ...
              (progressError.message.includes("function") ||
                progressError.code === "42883")
            ) {
              throw new Error("RPC_MISSING");
            }
    ...
    if (!result.success || !result.data) {
      const errorMessage =
        result.error?.message || "Analysis failed without error message";
    ...
    try {
      ...
      if (completeError) {
        if (
          ...
          (completeError.message.includes("function") ||
      ...
      if ((err as Error).message === "RPC_MISSING") {
        jobLogger.warn(
          "RPC function not found, falling back to manual completion",
        );
        const { error: updateError } = await this.supabase
          .from("analysis_jobs")
          .update({
        ...
        if (updateError)
          throw new Error(
            `Failed to complete job (fallback): ${updateError.message}`,
          );
      } else {
        throw err;
      }
  ...
  private async processInvoiceExtraction(job: AnalysisJob): Promise<void> {
    ...
    if (!result.success || !result.data) {
      const errorMessage =
        ...
        "Invoice extraction failed without error message";
      throw new Error(errorMessage);
    }
  ...
  private async processRegulatoryResearch(job: AnalysisJob): Promise<void> {
    ...
    if (!result.success || !result.data) {
      const errorMessage =
        result.error?.message ||
        "Regulatory research failed without error message";
      throw new Error(errorMessage);
    }
  ...
  private async processReportGeneration(job: AnalysisJob): Promise<void> {
    ...
    if (!result.success || !result.data) {
      const errorMessage =
        result.error?.message ||
        "Report generation failed without error message";
      throw new Error(errorMessage);
    }
