...
export function generateOptimization(
  ...
  if (recommended.length > 0) {
    ...
    sortedRecs.forEach((rec, index) => {
      ...
      if (rec.savings) {
        ...
        savingsDetailRow.getCell(2).font = {
          ...FONTS.bodyBold,
          color: { argb: COLORS.success },
        } as any;
  ...
  if (result.compactorOptimization && result.compactorOptimization.recommend) {
    mergeCells(
      ...
      currentRow,
      6,
      "Compactor Optimization - Detailed Breakdown",
      "section",
    );
    currentRow++;
  ...
  if (notRecommended.length > 0) {
    ...
    noteRow.getCell(1).value =
      "The following opportunities were evaluated but are not recommended at this time:";
  ...
  if (result.leaseUpDetected) {
    mergeCells(
      ...
      currentRow,
      6,
      "Important Note: Lease-Up Property",
      "section",
    );
    currentRow++;
