...
export function generateContractTerms(
  ...
  if (contract.property.units) {
    addKeyValueRow(
      ...
      worksheet.getRow(currentRow++),
      "Units:",
      contract.property.units.toString(),
    );
  }
  ...
  formatDate(
    reminderRow1.getCell(2),
    reminder90Date.toISOString().split("T")[0],
  );

  reminderRow1.getCell(3).value =
    "Begin contract renewal negotiation or vendor RFP process";
  ...
  formatDate(
    reminderRow2.getCell(2),
    reminder30Date.toISOString().split("T")[0],
  );

  reminderRow2.getCell(3).value =
    "Finalize contract renewal or transition plan";
  ...
  formatDate(
    reminderRow3.getCell(2),
    reminder7Date.toISOString().split("T")[0],
  );

  reminderRow3.getCell(3).value =
    "Confirm service transition or renewal is complete";
