...
export function generateExecutiveSummary(
  ...
  mergeCells(worksheet, currentRow, 1, currentRow, 6, project.property_name, 'header')
  applyHeaderStyle(headerRow)
  currentRow += 2

  // Analysis date
  const subtitleRow = worksheet.getRow(currentRow)
  subtitleRow.getCell(1).value = `WasteWise Analysis - Generated ${new Date().toLocaleDateString('en-US', {
    month: 'long',
    day: 'numeric',
    year: 'numeric',
  })}`
  ...
  mergeCells(worksheet, currentRow, 1, currentRow, 6, 'Property Information', 'section')
  currentRow++

  addKeyValueRow(worksheet, worksheet.getRow(currentRow++), 'Property Name:', project.property_name)
  addKeyValueRow(worksheet, worksheet.getRow(currentRow++), 'Address:', (project as any).property_address || 'N/A')
  addKeyValueRow(worksheet, worksheet.getRow(currentRow++), 'Units:', project.units.toString())
  addKeyValueRow(worksheet, worksheet.getRow(currentRow++), 'Property Type:', project.property_type || 'N/A')
  addKeyValueRow(worksheet, worksheet.getRow(currentRow++), 'Equipment Type:', project.equipment_type || 'N/A')
  addKeyValueRow(
    worksheet,
  ...
  // ========== KEY METRICS ==========
  mergeCells(worksheet, currentRow, 1, currentRow, 6, 'Key Performance Metrics', 'section')
  currentRow++
  ...
  mergeCells(worksheet, currentRow, 1, currentRow, 6, 'Benchmark Comparison', 'section')
  currentRow++
  ...
  mergeCells(worksheet, currentRow, 1, currentRow, 6, 'Top Recommendations', 'section')
  currentRow++
  ...
  // ========== SAVINGS SUMMARY ==========
  mergeCells(worksheet, currentRow, 1, currentRow, 6, 'Total Savings Potential', 'section')
  currentRow++
 ...
 */
function calculateYardsPerDoor(result: WasteWiseAnalyticsCompleteResult, project: ProjectRow): number {
  ...
  if (result.compactorOptimization) {
    ...
    const weeksInPeriod = calculateWeeks(result.summary.dateRange.start, result.summary.dateRange.end)
 ...
 */
function getBenchmark(propertyType: string): (typeof BENCHMARKS)[keyof typeof BENCHMARKS] | null {
