...
export function generateExpenseAnalysis(
  ...
  const monthsInPeriod = calculateMonths(
  ...
  formatCurrency(
    chargesTotalRow.getCell(4),
    chargesSummary.total / monthsInPeriod,
  );
  chargesTotalRow.getCell(4).font = FONTS.bodyBold as any;

  formatCurrency(
    chargesTotalRow.getCell(5),
    chargesSummary.total / monthsInPeriod / project.units,
  );
  ...
  const contaminationPct =
    (chargesSummary.contamination / chargesSummary.total) * 100;
  ...
  if (contaminationPct > 3) {
    ...
    contaminationRow.getCell(3).value =
      "Above 3% threshold - consider contamination reduction program";
 ...
 */
function getLargestCategory(summary: ChargesSummary): {
  name: string;
  amount: number;
  percentage: number;
} {
