...
import type { Worksheet, Cell, Row, Column, Border, Borders, Fill, Font, Alignment } from 'exceljs'
 ...
 */
export function formatCurrency(cell: Cell, value: number, decimals: boolean = true): void {
  cell.value = value
  cell.numFmt = decimals ? NUMBER_FORMATS.currency : NUMBER_FORMATS.currencyNoDecimals
 ...
 */
export function formatPercentage(cell: Cell, value: number, decimals: boolean = true): void {
  cell.value = value / 100 // ExcelJS expects decimal for percentage
  cell.numFmt = decimals ? NUMBER_FORMATS.percentage : NUMBER_FORMATS.percentageNoDecimals
  cell.alignment = ALIGNMENTS.right as Alignment
}
 ...
 */
export function formatNumber(cell: Cell, value: number, decimals: boolean = true): void {
  cell.value = value
  cell.numFmt = decimals ? NUMBER_FORMATS.decimal : NUMBER_FORMATS.decimalNoDecimals
  cell.alignment = ALIGNMENTS.right as Alignment
}
 ...
 */
export function autoSizeColumns(worksheet: Worksheet, minWidth: number = 10, maxWidth: number = 50): void {
 ...
 */
export function applyConditionalFormatting(cell: Cell, value: number, isSavings: boolean = true): void {
  if (isSavings) {
    // Positive is good (green), negative is bad (red)
    if (value > 0) {
      cell.fill = FILLS.highlightGreen as Fill
      cell.font = { ...FONTS.bodyBold, color: { argb: COLORS.success } } as Font
    } else if (value < 0) {
 ...
 */
export function addFooter(worksheet: Worksheet, startCol: number = 1, endCol: number = 6): void {
...
export function addKeyValueRow(
  ...
  if (formatType === 'currency' && typeof value === 'number') {
    ...
    if (typeof value === 'string' || (value && typeof value === 'object' && 'getDate' in value)) {
