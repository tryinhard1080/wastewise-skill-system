...
const TEST_CASES: TestCase<CompactorTestInput, CompactorExpectedOutput>[] = [
  ...
  },
  {
    ...
    description: 'Property with good utilization (6.5 tons/haul) - should NOT recommend',
    input: {
      units: 200,
      containerSizeCy: 30,
      haulLogData: [
  ...
  },
  {
    ...
    description: 'Edge case: 5.8 tons/haul (< 6.0 threshold) - should recommend',
    input: {
      units: 150,
      containerSizeCy: 30,
      haulLogData: [
  ...
  },
  {
    ...
    description: 'Edge case: 6.0 tons/haul (exactly at threshold) - should NOT recommend',
    input: {
      units: 180,
      containerSizeCy: 30,
      haulLogData: [
  ...
  },
  {
    ...
    description: 'Verify formula constants match WASTE_FORMULAS_REFERENCE.md v2.0',
    input: {
      units: 200,
      containerSizeCy: 30,
      haulLogData: [
  ...
  },
  {
    ...
    description: 'Verify max days between constraint (>14 days should NOT recommend)',
    input: {
      units: 200,
      containerSizeCy: 30,
      haulLogData: [
...
async function runSingleTest(
  ...
  const comparisons = compareMetrics(
    {
      avgTonsPerHaul: [tsOutput.avgTonsPerHaul, testCase.expected.avgTonsPerHaul],
    },
    DEFAULT_TOLERANCE
  )
  ...
  if (testCase.expected.currentAnnualHauls !== undefined && tsOutput.currentAnnualHauls) {
    comparisons.push(
      ...compareMetrics({
        currentAnnualHauls: [tsOutput.currentAnnualHauls, testCase.expected.currentAnnualHauls],
      })
    )
  }

  if (testCase.expected.optimizedAnnualHauls !== undefined && tsOutput.optimizedAnnualHauls) {
    comparisons.push(
      ...compareMetrics({
  ...
  if (testCase.expected.grossAnnualSavings !== undefined && tsOutput.grossAnnualSavings) {
    comparisons.push(
      ...compareMetrics({
        grossAnnualSavings: [tsOutput.grossAnnualSavings, testCase.expected.grossAnnualSavings],
      })
    )
  }
...
function buildSkillContext(input: CompactorTestInput): SkillContext {
  ...
  const invoices: InvoiceDataRow[] = input.invoiceData.map((invoice, index) => ({
    id: `invoice-${index}`,
    project_id: 'test-project',
    source_file_id: null,
 ...
 */
function generateWeeklyHauls(weeks: number, avgTons: number): Array<{ date: string; tonnage: number }> {
