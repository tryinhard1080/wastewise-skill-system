...
function formatValidationError(report: ValidationReport): string {
  ...
  lines.push("");
  lines.push(
    `${report.mismatches.length} constant(s) do not match WASTE_FORMULAS_REFERENCE.md v2.0:`,
  );
  lines.push("");
  ...
  lines.push(
    "CRITICAL: All formula constants must match the canonical reference.",
  );
  lines.push(
    "Please update lib/constants/formulas.ts to match WASTE_FORMULAS_REFERENCE.md v2.0",
  );
