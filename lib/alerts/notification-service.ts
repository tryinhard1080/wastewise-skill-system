...
export async function sendPagerDutyNotification(
  ...
  if (!process.env.PAGERDUTY_KEY) {
    logger.debug(
      "PagerDuty notifications disabled - PAGERDUTY_KEY not configured",
    );
    return false;
  }
...
function getEmailSubject(alert: JobAlert): string {
  ...
  const emoji =
    severityEmoji[alert.severity as keyof typeof severityEmoji] || "📢";
...
function getEmailHtml(alert: JobAlert, job?: AnalysisJob): string {
  ...
  html += `
          <div class="footer">
            ...
            <p>Timestamp: ${alert.created_at ? new Date(alert.created_at).toLocaleString() : "Unknown"}</p>
          </div>
        </div>
      </div>
    </body>
...
function getSlackPayload(
  ...
  const emoji =
    severityEmoji[alert.severity as keyof typeof severityEmoji] || ":bell:";
  ...
  blocks.push({
    ...
    elements: [
      {
        type: "mrkdwn",
        text: `Alert ID: ${alert.id} | ${alert.created_at ? new Date(alert.created_at).toLocaleString() : "Unknown"}`,
      },
    ],
  });
