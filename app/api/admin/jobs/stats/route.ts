...
async function handleGET(req: AdminRequest) {
  try {
    ...
    const stats = {
      ...
      pending:
        statusCounts?.filter((j: any) => j.status === "pending").length || 0,
      processing:
        statusCounts?.filter((j: any) => j.status === "processing").length || 0,
      completed:
        statusCounts?.filter((j: any) => j.status === "completed").length || 0,
      failed:
        statusCounts?.filter((j: any) => j.status === "failed").length || 0,
      cancelled:
        statusCounts?.filter((j: any) => j.status === "cancelled").length || 0,
    };
