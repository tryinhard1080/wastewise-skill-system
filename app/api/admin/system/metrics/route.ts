...
async function handleGET(req: AdminRequest) {
  try {
    ...
    const uniqueActiveUsers = new Set(activeJobs?.map((j: any) => j.user_id)).size

    // Jobs processed
    const { data: jobs } = await supabase
      .from('analysis_jobs')
      .select('status, created_at, ai_tokens_input, ai_tokens_output, ai_cost_usd, ai_requests')
      .gte('created_at', startDate.toISOString())
    ...
    const aiUsage = jobs?.reduce((acc: any, job: any) => {
      ...
      return acc
    }, { totalTokensInput: 0, totalTokensOutput: 0, totalCost: 0, apiCalls: 0 }) || { totalTokensInput: 0, totalTokensOutput: 0, totalCost: 0, apiCalls: 0 }
    ...
    const storageUsage = {
      ...
      totalBytes: files?.reduce((sum: number, f: any) => sum + (f.file_size || 0), 0) || 0,
      totalMB: Math.round((files?.reduce((sum: number, f: any) => sum + (f.file_size || 0), 0) || 0) / (1024 * 1024))
    }
    ...
    const projectStats = {
      ...
      processing: projects?.filter((p: any) => p.status === 'processing').length || 0,
