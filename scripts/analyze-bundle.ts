...
function main() {
  ...
  const pageBundles = analyzeBuildDirectory(
    join(staticDir, "chunks", "pages"),
    "page",
  );
  ...
  const chunkBundles = analyzeBuildDirectory(chunksDir, "chunk").filter(
    (b) => !b.name.includes("pages/"),
  ); // Exclude page chunks already analyzed
  ...
  console.log(
    `Large Bundles (>${report.largeThreshold}): ${report.largeBundles}`,
  );
