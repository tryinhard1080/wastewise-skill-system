...
class ValidationOrchestrator {
  ...
  private checkPrerequisites(): boolean {
    console.log(
      `${colors.cyan}${colors.bright}Checking Prerequisites...${colors.reset}\n`,
    );
  ...
  private displaySummary(): void {
    ...
    for (const result of this.results) {
      ...
      console.log(
        `  ${icon} Phase ${result.phase.id}: ${result.phase.name}... ${status} ${duration}`,
      );
    ...
    if (allPassed) {
      console.log(
        `${colors.green}${colors.bright}🎉 All validations passed! Ready for production.${colors.reset}`,
      );
      console.log();
      console.log(
        `${colors.dim}✅ WasteWise is production-ready${colors.reset}`,
      );
      console.log(
        `${colors.dim}✅ All critical user workflows tested${colors.reset}`,
      );
      console.log(
        `${colors.dim}✅ All calculations verified accurate${colors.reset}`,
      );
      console.log(
        `${colors.dim}✅ All security measures validated${colors.reset}`,
      );
      console.log(`${colors.dim}✅ Safe to deploy${colors.reset}`);
    } else {
      console.log(
        `${colors.red}${colors.bright}❌ Validation failed. Do NOT deploy to production.${colors.reset}`,
      );
      console.log();
  ...
  async run(): Promise<void> {
    ...
    console.log(
      `${colors.magenta}${colors.bright}WasteWise Validation Orchestrator${colors.reset}`,
    );
    console.log(
      `${colors.magenta}${colors.bright}${"=".repeat(60)}${colors.reset}`,
    );
    console.log();
    console.log(
      `${colors.dim}Philosophy: "If /validate passes, WasteWise is production-ready"${colors.reset}`,
    );
    console.log();
    ...
    if (this.specificPhase) {
      phasesToRun = VALIDATION_PHASES.filter(
        (p) => p.id === this.specificPhase,
      );
      if (phasesToRun.length === 0) {
        console.log(
          `${colors.red}❌ Invalid phase number: ${this.specificPhase}${colors.reset}\n`,
        );
        process.exit(2);
      }
      console.log(
        `${colors.yellow}Running specific phase: ${this.specificPhase}${colors.reset}\n`,
      );
    }
    ...
    if (this.skipE2E) {
      ...
      console.log(
        `${colors.yellow}Skipping E2E tests (--skip-e2e flag)${colors.reset}\n`,
      );
    }
