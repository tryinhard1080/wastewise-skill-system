    ...
    const instance = autocannon(
      ...
      (err, result) => {
        if (err) {
          ...
          console.log(
            `   Avg Latency: ${formatted.latency.average.toFixed(2)}ms`,
          );
...
async function main() {
  ...
  for (const scenario of scenarios) {
    try {
      const result = await runLoadTest(scenario.name, scenario.url, {
  ...
  const summary = {
    ...
    results: results.map((r) => ({
      ...
      errors: r.errors,
      errorRate:
        (((r.errors + r.non2xx) / r.requests.total) * 100).toFixed(2) + "%",
    })),
  };
  ...
  const heavyLoadScenario = results.find((r) =>
    r.scenario.includes("Heavy Load (100 concurrent)"),
  );
  if (heavyLoadScenario) {
    const p95Latency = heavyLoadScenario.latency.p95;
    const errorRate =
      (heavyLoadScenario.errors + heavyLoadScenario.non2xx) /
      heavyLoadScenario.requests.total;
