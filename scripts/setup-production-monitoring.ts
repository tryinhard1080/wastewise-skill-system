...
class ProductionMonitoringSetup {
  ...
  private loadConfig(): MonitoringConfig {
    ...
    if (!fs.existsSync(configPath)) {
      throw new Error('.env.production file not found. Create it with all required variables.')
    }
  ...
  private async setupSentry(): Promise<void> {
    ...
    try {
      execSync(`sentry-cli releases new ${this.config.sentry.release}`, { stdio: 'pipe' })
      console.log(`✅ Sentry release created: ${this.config.sentry.release}`)

      // Upload source maps
      execSync('sentry-cli releases files upload-sourcemaps ./.next/static', { stdio: 'pipe' })
      console.log('✅ Source maps uploaded to Sentry')

      // Finalize release
      execSync(`sentry-cli releases finalize ${this.config.sentry.release}`, { stdio: 'pipe' })
  ...
  private async setupUptimeMonitoring(): Promise<void> {
    ...
    console.log('✅ Uptime monitoring configuration saved to ./config/uptime-monitors.json')
  ...
  private async setupAlertChannels(): Promise<void> {
    ...
    if (this.config.alerts.email.length > 0) {
      console.log(`✅ Email alerts configured for: ${this.config.alerts.email.join(', ')}`)
    }
...
`
    ...
    for (const check of healthChecks) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`⚠️  ${check.name} health check returned ${response.status}`)
        }
      } catch (error) {
        console.log(`❌ ${check.name} health check not accessible (deploy first)`)
      }
    }
  ...
  private async verifySetup(): Promise<void> {
    ...
    checks.forEach(check => {
      console.log(`${check.passed ? '✅' : '⚠️ '} ${check.name}: ${check.value}`)
    })
