...
async function main() {
  ...
  const avgTons = haulLogEntries.reduce((sum, e) => sum + e.tonnage, 0) / haulLogEntries.length
  console.log(`✅ Created ${haulLogEntries.length} haul log entries`)
  console.log(`   Average tons per haul: ${avgTons.toFixed(2)} (< 6.0, should recommend monitoring)\n`)
  ...
  while (attempts < maxAttempts) {
    ...
    if (currentJob) {
      console.log(`   [${attempts + 1}s] Status: ${currentJob.status} | Progress: ${currentJob.progress_percent}% | Step: ${currentJob.current_step || 'N/A'}`)
  ...
  console.log('Results:')
  console.log(`   Recommend monitoring: ${resultData.recommend ? '✅ YES' : '❌ NO'}`)
  console.log(`   Average tons per haul: ${resultData.avgTonsPerHaul?.toFixed(2)} tons`)
  ...
  console.log(`   Gross annual savings: $${resultData.grossAnnualSavings?.toFixed(2)}`)
  console.log(`   Net Year 1 savings: $${resultData.netYear1Savings?.toFixed(2)}`)
  console.log(`   Net annual savings (Y2+): $${resultData.netAnnualSavingsYear2Plus?.toFixed(2)}`)
  console.log(`   ROI: ${resultData.roiPercent?.toFixed(2)}%`)
  console.log(`   Payback period: ${resultData.paybackMonths?.toFixed(1)} months`)
