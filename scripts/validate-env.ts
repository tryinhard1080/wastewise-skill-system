 ...
 */
function validateEnvVar(envVar: EnvVariable): { errors: string[]; warnings: string[] } {
  const errors: string[] = []
 ...
 */
function validateEnvironmentSpecific(): { errors: string[]; warnings: string[] } {
  const errors: string[] = []
  const warnings: string[] = []
  ...
  if (nodeEnv === 'production') {
    if (!process.env.NEXT_PUBLIC_SENTRY_DSN) {
      warnings.push('⚠️  Sentry DSN not configured for production (error tracking recommended)')
    }
    ...
    if (process.env.LOG_LEVEL === 'debug') {
      warnings.push('⚠️  LOG_LEVEL set to debug in production (should be warn or error)')
    }
 ...
 */
function checkCommonMisconfigurations(): { errors: string[]; warnings: string[] } {
  const errors: string[] = []
  const warnings: string[] = []

  // Check if using example/placeholder values
  const examplePatterns = ['your-', 'example', 'placeholder', 'changeme', 'localhost']
  ...
  if (
    ...
    errors.push('❌ ANON_KEY and SERVICE_ROLE_KEY are the same (they should be different)')
  }
