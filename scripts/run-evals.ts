...
} from "../lib/evals/formula-validator";
...
async function main() {
  ...
  try {
    ...
    if (constantReport.mismatches.length > 0) {
      ...
      console.error(
        "   Please ensure lib/constants/formulas.ts matches WASTE_FORMULAS_REFERENCE.md",
      );
      hasErrors = true;
    } else {
  ...
  try {
    ...
    console.log(
      `Failed:       ${evalResults.failing} ${evalResults.failing > 0 ? "❌" : ""}`,
    );
    ...
    if (evalResults.failing > 0) {
      ...
      console.error(
        "   Review the failed tests above and ensure formulas are correct.",
      );
      hasErrors = true;
    } else {
      console.log("\n✅ ALL EVALS PASSED!");
...
function printSkillSummary(summary: EvalSummary) {
  ...
  console.log(
    `${status} Skill: ${summary.results[0]?.description.split(" - ")[0] || "Unknown"}`,
  );
      ...
      .forEach((r) => {
        ...
        r.comparisons
          ...
          .forEach((c) => {
            ...
            console.log(
              `       ${c.metric}: ${c.tsValue} vs ${c.pythonValue} (${percentDiff}% diff)`,
            );
          });
      });
  }
