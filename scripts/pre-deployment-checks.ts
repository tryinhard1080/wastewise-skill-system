...
class DeploymentValidator {
  ...
  private async checkDatabase(): Promise<void> {
    ...
    this.addResult({
      ...
      message: hasRPCFunctions ? 'Database functions defined' : 'No RPC functions found',
      duration: 0
    })
  ...
  private async checkSecurity(): Promise<void> {
    ...
    await this.runCheck(
      ...
      () => {
        ...
        for (const file of files) {
          ...
          if (/eyJ[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+/.test(content) && !file.includes('__tests__')) {
    ...
    this.addResult({
      ...
      message: hasRateLimiting ? 'Rate limiting configured' : 'Rate limiting not found',
      duration: 0
    })
