...
const MAX_WIDTH_WITHOUT_RESPONSIVE =
  /className="[^"]*\bmax-w-\[(\d+)px\][^"]*"/g;
    ...
    if (TABLE_WITHOUT_SCROLL.test(line)) {
      ...
      if (!hasOverflow) {
        issues.push({
          ...
          line: lineNumber,
          issue:
            "Table should be wrapped in overflow-x-auto container for mobile",
          code: line.trim(),
          severity: "warning",
        });
      }
    ...
    if (line.includes("flex-row") && !line.includes("flex-col")) {
      if (!line.match(/flex-col\s+(?:sm|md|lg):flex-row/)) {
        issues.push({
          ...
          line: lineNumber,
          issue:
            "flex-row might need flex-col on mobile (flex-col sm:flex-row)",
          code: line.trim(),
          severity: "info",
        });
      }
